...
есть указанное количество умножается на число десятичных знаков на уровне API, чтобы иметь возможность работать только с целыми значениями.  |
| Reissuable | \[Возможность дополнительного выпуска, определяет, можно ли позднее довыпустить ассеты\] Boolean - флаг, определяющий
...
| Поле | Значение поля |
| :--- | :--- |
| Issuer | \[Waves адрес, который создал ассет\] Array\[Byte\] - txid транзакции выпуска. |
...
| Поле | Значение поля |
| :--- | :--- |
| Issuer | \[Waves адрес, с которого ассет был создан\] Array\[Byte\] - txid транзакции выпуска. |
